...
RSpec.describe TrafficLight, type: :model do
  fixtures :intersections, :traffic_lights
  let(:intersection)  { intersections(:zalg_kalv) }
  let(:traffic_light) { traffic_lights(:one)      }

  before do
    traffic_light.intersection = intersection
  end

  it 'belongs to an intersection' do
    expect(traffic_light.intersection).to eq(intersection)
  end
